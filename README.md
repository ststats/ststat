...
운영에서는 `.github/workflows/pipeline.yml`을 외부 스케줄러가 호출합니다. 작업들은 실제 의존성에 따라 분리되어 Poonggo나 YouTube 한 곳의 장애가 다른 독립 작업을 막지 않습니다.
...
- 2026-09-24 경기 삭제 사고는 백업 JSON으로, 2026-09-25에는 ID 역전으로 빠진 경기 177건을 EloBoard 전체 재수집으로
  복구했고 잘못 저장된 경기 20건(승자=패자 14·선수 번호 없음 6)을 백업 후 지웠다. 복구용 코드(전체 재수집 모드·재집계·점검·정리
  워크플로)는 끝나서 지웠다 - 다시 필요하면 git 기록에서 꺼낸다(`b38249f`, `46df393`).
