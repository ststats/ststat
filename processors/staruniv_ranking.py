...
[현재 사양] - 값을 바꾸면 여기와 해당 상수 주석을 같이 고친다
  ...
  · 형식 가중치 CAT_WEIGHT: 개인·대학대회 1.0 > 대학대전 0.9 > 미니 0.8 > 프로리그·CK 0.7 > 스폰·기타 0.6
  · 순위 = 티어 안 θ 순서. 최근 RECENT_DAYS(365일)에 MIN_RECENT_GAMES(10)판 미만이면 '기록 없음'
  · δ prior 폭 SIGMA_DELTA = 0.675, 데이터 티어 판정 DATA_TIER_Z = 1.5
  · 종족 상성 3개(TZ·ZP·PT)를 m과 함께 긴 창에서 맞춘다
아래 [...] 절들은 이 구조를 택한 이유다. 그 안의 경기 수·점수·로그손실은 **당시 실측값**이고,
반감기·가중치 같은 설정값은 이후 재튜닝으로 바뀐 것이 있다(지금 값은 위 목록과 상수가 기준).
...
(스페이드 vs 0티어 1,204경기에서 스페이드 54.1% 승 = 약 +28점)과 비교하면:
    ...
    + m을 긴 창(2년)으로 따로 맞춤     스페이드 - 0티어 =  +24점   (아래 [두 반감기]. 지금 540일 창은 +28.3점)
...
그래서 m은 긴 창으로, δ는 짧은 창으로 따로 맞춘다(처음엔 2년/4개월, 지금은 540일/90일).
자세한 수치는 HALF_LIFE_DAYS·HALF_LIFE_TIER_DAYS 주석에 적어뒀다. 이 2단을 넣고서야 티어 기준선 15칸이 처음으로
...
#     표준오차가 작아져 유리해진다(당시 순위 점수가 θ-SE였다). 그래서 순서는 지키되
...
# δ에 거는 prior의 폭(표준편차). 로짓 단위에서 티어 한 칸 차이는 약 0.36이다(실측 1칸 차 승률 58.8%).
# 처음 0.4/0.5/0.65를 미래 90일 동티어 경기로 비교했고, 전체 구조를 다시 튜닝한 뒤
# 0.625/0.65/0.675/0.70/0.725를 미래 예측·실제 승급자 상위권 포착·저표본 억제 기준으로 재비교해
# 0.675가 로그손실과 승급 포착의 균형이 가장 좋았다. 데이터 티어 배수(DATA_TIER_Z 1.50)는 그대로.
SIGMA_DELTA = 0.675
